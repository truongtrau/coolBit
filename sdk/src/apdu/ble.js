...
export const assemblyBytes = (cla, ins, p1, p2, oriData) => {
  let pid = '00',
    cmd_len = '09';
  ...
  // console.log(`p1:${p1},p2:${p2},oriData:${oriData}`)
  if (!!oriData) {
    packets = oriData;
  ...
  oriData_length = Helper.Other.fitZero(oriData_length.toString('hex'), 4, '0', true)

  const XORData = Buffer.allocUnsafe(4)
  XORData.fill(0)
  XORData.writeInt16BE(XOR_length, 0)
  XOR_length = XORData.slice(0, 2).toString('hex')
  XOR_length = Helper.Other.fitZero(XOR_length.toString('hex'), 4, '0', true)
